...
namespace TestRail
{
    ...
    public class TestRailClient
    {
        ...
        private CommandResult _CallTestRailGetEndpoint(string uri)
        {
            ...
            var authInfo = Convert.ToBase64String(Encoding.GetEncoding(0).GetBytes($"{_UserName_}:{_Password_}"));
            var authValue = new AuthenticationHeaderValue("Basic", authInfo);

            using (var client = new HttpClient()
            {
                BaseAddress = new Uri(_URL_),
            ...
            })
            {
                try
                {
                    ...
                    cr = new CommandResult(response.StatusCode == HttpStatusCode.OK, res);
                }
